...
class Warehouse(TenantBaseModel, SoftDeleteMixin):
    ...
    code = Column(String(50), nullable=True)  # Short code like WH-01
...
class Stock(TenantBaseModel):
    ...
    __table_args__ = (
        UniqueConstraint('tenant_id', 'product_id', 'warehouse_id', name='uq_stock_tenant_product_warehouse'),
...
class InventoryCheck(TenantBaseModel):
    """
...
class InventoryCheckItem(TenantBaseModel):
    """
    ...
    __table_args__ = (
        UniqueConstraint('tenant_id', 'inventory_check_id', 'product_id', name='uq_inventory_item_tenant'),
