...
async def get_products(
    ...
    # Pre-load stock data for warehouse filtering (more reliable than dynamic relationship)
    from database.models import Stock as StockModel
    ...
    data = []
    for p in products:
        # Get current stock — filtered by warehouse if specified
        try:
            if warehouse_id:
                stock_rows = db.query(StockModel).filter(
                    StockModel.product_id == p.id,
                    StockModel.warehouse_id == warehouse_id,
                    StockModel.tenant_id == tenant_id
                ).all()
                current_stock = sum(s.quantity for s in stock_rows) if stock_rows else Decimal("0")
            else:
