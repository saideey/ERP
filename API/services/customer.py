...
class CustomerService(TenantServiceBase):
    """Customer management service."""
    ...
    def get_customer_by_id(self, customer_id: int) -> Optional[Customer]:
        """Get customer by ID."""
        return self._q(Customer).filter(
    ...
    def get_customers(
        ...
        query = self._q(Customer).options(
    ...
    def get_debt_history(
        ...
        query = self._q(CustomerDebt).filter(
    ...
    def get_total_debt(self, manager_id: int = None) -> Decimal:
        ...
        query = self._q(Customer).with_entities(func.sum(Customer.current_debt)).filter(
            Customer.is_deleted == False
        )
    ...
    def set_vip_credentials(
        ...
        existing = self._q(Customer).filter(
            Customer.login == login,
            Customer.id != customer_id
        ).first()
        if existing:
    ...
    def _send_payment_notification(
        ...
        try:
            ...
            if created_by_id:
                user = self._q(User).filter(User.id == created_by_id).first()
                if user:
