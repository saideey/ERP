...
class ProductService(TenantServiceBase):
    ...
    def get_products(
        ...
        query = self._q(Product).filter(Product.is_deleted == False)
    ...
    def create_product(
        ...
        ok, limit_msg = check_limit(self.db, self.tenant_id, LimitType.PRODUCTS)
        if not ok:
        ...
        base_uom = self._q(UnitOfMeasure).filter(
    ...
    def update_product(
        ...
        if 'base_uom_id' in update_data and update_data['base_uom_id'] is not None:
            new_uom = self._q(UnitOfMeasure).filter(
    ...
    def add_uom_conversion(
        ...
        existing = self._q(ProductUOMConversion).filter(
    ...
    def get_product_stock(self, product_id: int, warehouse_id: int = None) -> List[Stock]:
        ...
        query = self._q(Stock).filter(Stock.product_id == product_id)
...
class CategoryService(TenantServiceBase):
    ...
    def get_category_by_id(self, category_id: int) -> Optional[Category]:
        """Get category by ID."""
        return self._q(Category).filter(
    ...
    def get_categories(
        ...
        """Get categories list."""
        query = self._q(Category).filter(Category.is_deleted == False)
    ...
    def get_category_tree(self) -> List[dict]:
        ...
        categories = self._q(Category).filter(
            Category.is_deleted == False,
    ...
    def create_category(
        ...
        existing = self._q(Category).filter(Category.slug == slug).first()
        if existing:
    ...
    def delete_category(self, category_id: int, deleted_by_id: int) -> Tuple[bool, str]:
        ...
        children = self._q(Category).filter(
        ...
        # Check for products
        products = self._q(Product).filter(
...
class UOMService(TenantServiceBase):
    ...
    def get_all_uoms(self, uom_type: Optional[str] = None) -> List[UnitOfMeasure]:
        ...
        query = self._q(UnitOfMeasure).filter(UnitOfMeasure.is_active == True)
    ...
    def get_uom_by_id(self, uom_id: int) -> Optional[UnitOfMeasure]:
        """Get UOM by ID."""
        return self._q(UnitOfMeasure).filter(UnitOfMeasure.id == uom_id).first()
    ...
    def create_uom(
        ...
        # Check uniqueness
        existing = self._q(UnitOfMeasure).filter(
