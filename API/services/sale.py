...
class SaleService(TenantServiceBase):
    ...
    def get_sale_by_id(self, sale_id: int) -> Optional[Sale]:
        ...
        return self._q(Sale).filter(Sale.id == sale_id).first()
    
    def get_sale_by_number(self, sale_number: str) -> Optional[Sale]:
        """Get sale by sale number."""
        return self._q(Sale).filter(Sale.sale_number == sale_number).first()
    ...
    def get_sales(
        ...
        query = self._q(Sale).filter(Sale.is_cancelled == is_cancelled)
    ...
    def create_sale(
        ...
        for item_data in items:
            product = self._q(Product).filter(
    ...
    def cancel_sale(
        ...
        if return_to_stock:
            for item in sale.items:
                ...
                unit_cost = stock.average_cost if stock and stock.average_cost else (item.unit_cost or Decimal("0"))
    ...
    def _send_purchase_notification(
        ...
        try:
            ...
            seller = self._q(User).filter(User.id == seller_id).first()
            ...
            # Send notification (fire-and-forget) with tenant context
