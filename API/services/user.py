...
class UserService(TenantServiceBase):
    ...
    def get_users(
        ...
        """
        query = self._q(User).filter(User.is_deleted == False)
    ...
    def create_user(self, data: UserCreate, created_by_id: int) -> Tuple[User, str]:
        ...
        ok, limit_msg = check_limit(self.db, self.tenant_id, LimitType.USERS)
        if not ok:
        ...
        if data.email:
            email_exists = self._q(User).filter(
        ...
        # Check role exists
        role = self._q(Role).filter(Role.id == data.role_id).first()
        if not role:
...
class RoleService(TenantServiceBase):
    ...
    def get_all_roles(self, include_inactive: bool = False) -> List[Role]:
        """Get all roles."""
        query = self._q(Role).filter(Role.is_deleted == False)
    ...
    def create_role(self, data: dict, created_by_id: int) -> Tuple[Optional[Role], str]:
        ...
        existing = self._q(Role).filter(Role.name == data["name"]).first()
        if existing:
    ...
    def delete_role(self, role_id: int, deleted_by_id: int) -> Tuple[bool, str]:
        ...
        users_count = self._q(User).filter(
