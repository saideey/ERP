...
"""
from services.base import TenantServiceBase
...
class WarehouseService(TenantServiceBase):
    ...
    def get_warehouse_by_id(self, warehouse_id: int) -> Optional[Warehouse]:
        """Get warehouse by ID."""
        return self._q(Warehouse).filter(
    ...
    def get_warehouses(self, include_inactive: bool = False) -> List[Warehouse]:
        """Get all warehouses."""
        query = self._q(Warehouse).filter(Warehouse.is_deleted == False)
    ...
    def create_warehouse(
        ...
        ok, limit_msg = check_limit(self.db, self.tenant_id, LimitType.WAREHOUSES)
        if not ok:
            return None, limit_msg
        
        # Check name uniqueness
        existing = self._q(Warehouse).filter(
            Warehouse.name == name,
            Warehouse.is_deleted == False
        ).first()
        if existing:
...
class StockService(TenantServiceBase):
    ...
    def get_all_stock(
        ...
        query = self._q(Stock).join(Product).filter(Product.is_deleted == False)
    ...
    def convert_to_base_uom(
        ...
        product = self._q(Product).filter(Product.id == product_id).first()
        if not product:
        ...
        # Find conversion
        conversion = self._q(ProductUOMConversion).filter(
    ...
    def get_movements(
        ...
        if search:
            ...
            matching_products = self._q(Product).with_entities(Product.id, Product.name).filter(
                or_(
                    Product.name.ilike(search_term),
                    Product.article.ilike(search_term)
                )
...
class StockTransferService(TenantServiceBase):
    ...
    def create_transfer(
        ...
        # Validate warehouses
        from_wh = self._q(Warehouse).filter(Warehouse.id == from_warehouse_id).first()
        to_wh = self._q(Warehouse).filter(Warehouse.id == to_warehouse_id).first()
        ...
        for item in items:
            ...
            if available < base_qty:
                self.db.rollback()
                product = self._q(Product).filter(Product.id == item["product_id"]).first()
    ...
    def complete_transfer(
        ...
        transfer = self._q(StockTransfer).filter(
