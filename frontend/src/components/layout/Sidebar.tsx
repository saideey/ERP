...
interface NavItem {
  nameKey: 'dashboard' | 'pos' | 'sales' | 'products' | 'customers' | 'warehouse' | 'reports' | 'users' | 'settings' | 'partners'
  href: string
...
// Path segments - will be prefixed with /${tenantSlug}
          ...
          <div className="flex items-center gap-3">
            {user?.tenant_logo_url ? (
              <img
                ...
                className="w-10 h-10 lg:w-12 lg:h-12 rounded-xl object-contain bg-gray-50 flex-shrink-0"
              />
            ) : (
              <div className="w-10 h-10 lg:w-12 lg:h-12 bg-blue-600 rounded-xl flex items-center justify-center flex-shrink-0">
                <span className="text-lg lg:text-xl font-bold text-white">
                  {user?.tenant_name?.charAt(0)?.toUpperCase() || 'M'}
                </span>
              </div>
            )}
            <div className="min-w-0">
              <p className="font-bold text-sm lg:text-base text-gray-900 truncate">
                {user?.tenant_name || 'X ERP'}
              </p>
              <p className="text-xs text-gray-500">ERP tizimi</p>
            </div>
          </div>
